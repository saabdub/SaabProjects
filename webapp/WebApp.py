...
def init_clienttable():
    ...
    db = connect_db()
    db.execute('create table if not exists clienttable (id integer primary key, email text, password text, name text, address text, phone text)')
    db.commit()
...
def login():
    if request.method == 'POST':
        ...
        data["password"] = data["password"].__hash__()
